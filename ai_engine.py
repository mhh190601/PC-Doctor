...
def format_answer(raw: dict) -> dict:
    ...
    out.update({
        ...
        # 自学习标记：前端展示"已自动学习，下次秒答"
        "learned": raw.get("learned", False),
    })
    return out
...
class AIEngine:
    ...
    def ask(self, question: str, mode: str = "auto") -> dict:
        ...
        # auto 模式：本地未命中/低分 → 联网搜索并自动学习
        # 如果语义检索有结果但分数低于阈值（0.1-0.7之间），也说明本地知识不够精准，触发联网
        web_result = self._web_search(question)
        if web_result:
            return web_result
        if semantic_hit:
        ...
        if result:
            ...
            learned = False
            if self.config["learning"]["auto_learn"]:
                kid = KnowledgeBridge.add_knowledge(question, answer, source="ai_cloud")
                if kid:
                    self.semantic.add_document(kid, question, answer, 0.7,
                                               source="ai_cloud", tags="云端学习", severity="中")
                    self.keyword.add_document(kid, question, answer, 0.7,
                                              source="ai_cloud", tags="云端学习", severity="中")
                    self.tag_matcher.add_document(kid, question, answer, 0.7,
                                                  source="ai_cloud", tags="云端学习", severity="中")
                    learned = True
            return format_answer({
                "success": True, "answer": answer,
    ...
    @staticmethod
    def _clean_web_result(text: str) -> str:
        """清洗联网搜索结果：去掉广告、太短的句子、无效行"""
        if not text:
            return text
        lines = text.split('\n')
        cleaned_lines = []
        ad_keywords = ['点击购买', '限时优惠', '广告', '加微信', '扫码关注',
                       '立即下载', '免费领取', '优惠券', '抢购', '秒杀']
        for line in lines:
            ...
            if len(line_stripped) < 10:
                continue
            # 跳过广告行
            if any(ad_word in line_stripped for ad_word in ad_keywords):
                continue
            # 跳过纯链接行
            if line_stripped.startswith('http') and len(line_stripped.split()) == 1:
                continue
    ...
    def _web_search(self, question: str) -> Optional[dict]:
        ...
        搜索结果自动清洗 + 缓存到本地知识库（自学习），下次同样问题可直接本地命中"""
        if not self.web_searcher:
            return None
        t0 = time.time()
        raw_text = self.web_searcher.search(question, count=3)
        if raw_text:
            logger.info(f"命中联网搜索: {question[:40]}...")
            _query_stats["cloud_hits"] += 1  # 复用 cloud 计数
            # 清洗搜索结果
            text = self._clean_web_result(raw_text)
            # 搜索结果自动存入本地知识库（标记 source=ai_web_learned，初始权重 0.8）
            learned = False
            if self.config["learning"]["auto_learn"]:
                try:
                    kid = KnowledgeBridge.add_knowledge(
                        question, text, source="ai_web_learned",
                        tags="自学习,联网搜索", severity="中", weight=0.8
                    )
                    if kid:
                        self.semantic.add_document(kid, question, text, 0.8,
                                                   source="ai_web_learned", tags="自学习,联网搜索", severity="中")
                        self.keyword.add_document(kid, question, text, 0.8,
                                                  source="ai_web_learned", tags="自学习,联网搜索", severity="中")
                        self.tag_matcher.add_document(kid, question, text, 0.8,
                                                      source="ai_web_learned", tags="自学习,联网搜索", severity="中")
                        learned = True
                        logger.info(f"自学习入库成功: id={kid}, source=ai_web_learned, weight=0.8")
                except Exception as e:
                    logger.warning(f"自学习入库失败: {e}")
            return format_answer({
                "success": True,
                "answer": text,
                "layer": "web_search",
